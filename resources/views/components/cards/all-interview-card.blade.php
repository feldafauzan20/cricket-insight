...
<a href="{{ route('news.show', ['locale' => app()->getLocale(), 'slug' => $article->slug]) }}"
    ...
    <div
        ...
        <div class="mb-2.75 flex items-center gap-x-2.5">
            <span class="text-xs font-semibold text-[#EC0226]">{{ $article->category?->name ?? 'Interview' }}</span>
            <div class="h-5 w-0.5 bg-[#A2A6A9]"></div>
            <p class="text-xs font-semibold text-[#A2A6A9]">{{ optional($article->published_at)->format('F d, Y') ?? 'N/A' }}</p>
        </div>
        <div class="md:w-110.25">
            <h1 class="text-xs font-semibold text-[#121212] md:text-lg dark:text-white">
