...
<div class="md:flex md:gap-x-7.5 border-b border-[#EFEFEF] dark:border-[#DEDEDE] pb-7.5">
    <div class="h-47.5 sm:w-69.25 lg:w-92.75 2xl:w-158 md:h-auto 2xl:h-49 rounded-[5px] overflow-hidden mb-3.75 md:mb-0">
        <img src="{{ $article->thumbnail ? asset('storage/' . $article->thumbnail) : asset('images/dummy/news-card/dummy-news-card.webp') }}" class="w-full h-full object-cover"
            alt="{{ $article->title }}">
    </div>
    <div class="2xl:w-full">
        <div class="bg-[#D6111A] w-fit py-1.25 px-3 rounded-[3px] mb-1.25">
            <p class="text-white font-medium text-[10px]">{{ $article->category?->name ?? 'News' }}</p>
        </div>
        ...
        <div class="flex items-center gap-x-2.25 mb-2.5">
            ...
            <span class="font-semibold text-[10px] text-[#666]">{{ optional($article->published_at)->format('d M Y') ?? 'N/A' }}</span>
        </div>
        ...
        <div class="flex justify-between items-center 2xl:max-h-full">
            <div class="flex items-center text-[#121212] dark:text-white gap-x-2.5">
                ...
                <p class="font-medium text-[10px]">BY {{ strtoupper($article->uploader?->name ?? 'UNKNOWN') }}</p>
            </div>
            <div class="flex items-center gap-x-1.25">
                <div class="flex items-center gap-x-1.25">
                    <x-bi-eye class="w-2.5 h-2.5 text-[#EC0226]" />
                    <span class="font-medium text-[10px] text-[#121212] dark:text-white">{{ number_format($article->views_count) }}</span>
                </div>
                <div class="rounded-full w-1.25 h-1.25 bg-[#666666]"></div>
                <div>
                    <p class="font-medium text-[10px] text-[#121212] dark:text-white">{{ ceil(str_word_count($article->description ?? strip_tags($article->content ?? '')) / 200) }} Mins Read</p>
                </div>
            </div>
        </div>
    </div>
