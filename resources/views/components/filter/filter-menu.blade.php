<div class="flex-col gap-4 2xl:flex" x-data="{
    applyFilter(key, value) {
        ...
        const url = new URL('{{ route('news.index', ['locale' => app()->getLocale()]) }}', window.location.origin);
    ...
    <div
        class="gap-1.25 mb-3.75 md:w-150 flex flex-wrap items-center border-[#C7C7C7] 2xl:mb-0 2xl:mr-3 2xl:w-fit 2xl:border-r-2 2xl:pr-3">
        <div class="relative" x-data="{ open: false }" @click.away="open = false"
            @filter-open.window="if ($event.detail !== 'sort') open = false">
            <x-filter.filter-card icon="ri-sort-desc" category="Sort" categoryValue="{{ $sortLabel }}"
                iconColor="#EC0226"
            ...
            <div x-show="open" x-cloak
                ...
                @foreach ($sortOptions as $sortKey => $sortLabelItem)
                    <button type="button"
                        ...
                        @click.prevent="applyFilter('sort', '{{ $sortKey }}'); open = false">{{ $sortLabelItem }}</button>
                @endforeach
            </div>
        </div>

        <div class="relative" x-data="{ open: false }" @click.away="open = false"
            @filter-open.window="if ($event.detail !== 'time_frame') open = false">
            <x-filter.filter-card icon="ri-time-line" category="Time Frame" categoryValue="{{ $timeLabel }}"
                iconColor="#EC0226"
            ...
            <div x-show="open" x-cloak
                ...
                @foreach ($timeFrames as $timeKey => $timeLabelItem)
                    <button type="button"
                        class="flex w-full items-center rounded-[6px] px-3 py-2 text-left text-sm text-[#121212] transition hover:bg-[#F3F3F3] dark:text-white dark:hover:bg-[#2A2A2A]"
                        @click.prevent="applyFilter('time_frame', '{{ $timeKey }}'); open = false">{{ $timeLabelItem }}</button>
                @endforeach
            </div>
        </div>

        <div class="relative" x-data="{ open: false }" @click.away="open = false"
            @filter-open.window="if ($event.detail !== 'popularity') open = false">
            <x-filter.filter-card icon="ri-fire-line" category="Popularity" categoryValue="{{ $popularityLabel }}"
                iconColor="#EC0226"
            ...
            <div x-show="open" x-cloak
                ...
                <button type="button"
                    class="flex w-full items-center rounded-[6px] px-3 py-2 text-left text-sm text-[#121212] transition hover:bg-[#F3F3F3] dark:text-white dark:hover:bg-[#2A2A2A]"
                    @click.prevent="applyFilter('popularity', ''); open = false">Any Popularity</button>
    ...
    <div class="border-t border-dashed border-[#E0E0E0] 2xl:border-none">
        <div class="pt-3.75 2xl:pt-0">
            <div class="relative" x-data="{ open: false }" @click.away="open = false"
                @filter-open.window="if ($event.detail !== 'tag') open = false">
                <x-filter.filter-card icon="ri-price-tag-3-line" category="Tag" categoryValue="{{ $tagLabel }}"
                    iconColor="#007DFC"
                ...
                <div x-show="open" x-cloak
                    ...
                    <button type="button"
                        class="flex w-full items-center rounded-[6px] px-3 py-2 text-left text-sm text-[#121212] transition hover:bg-[#F3F3F3] dark:text-white dark:hover:bg-[#2A2A2A]"
                        @click.prevent="applyFilter('tag', ''); open = false">Any Tag</button>
                    @foreach ($tags as $tag)
                        <button type="button"
                            class="flex w-full items-center rounded-[6px] px-3 py-2 text-left text-sm text-[#121212] transition hover:bg-[#F3F3F3] dark:text-white dark:hover:bg-[#2A2A2A]"
                            @click.prevent="applyFilter('tag', '{{ $tag->slug }}'); open = false">{{ $tag->name }}</button>
                    @endforeach
                </div>
            </div>
        </div>
