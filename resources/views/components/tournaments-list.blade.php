...
{{-- ON GOING TOURNAMENT CARDS LIST --}}
<div class="gap-y-6.25 flex flex-col">
    @if(count($tournaments) > 0)
        @foreach ($tournaments as $item)
            @php
                $title = $item->tournament_title ?? 'Ongoing Tournament';
                $timeStr = isset($item->time_date) ? \Carbon\Carbon::parse($item->time_date)->format('M d, H:i A T') : 'Live';
                $dateDay = isset($item->time_date) ? \Carbon\Carbon::parse($item->time_date)->format('D, M d') : '';
                $timeHour = isset($item->time_date) ? \Carbon\Carbon::parse($item->time_date)->format('h:i A') : '';
                $link = $item->redirect_link ?? '#';
                $desc = !empty($item->description) ? strip_tags($item->description) : 'Match yet to begin';
            @endphp
            <div>
                <div
                    ...
                    <a href="{{ $link }}" @if(!empty($item->redirect_link)) target="_blank" rel="noopener noreferrer" @endif class="text-xs font-medium text-[#EC0226] hover:underline">View Details &rarr;</a>
                </div>
                <div
                    ...
                    <div>
                        <div class="flex items-center gap-x-2.5">
                            @if(!empty($item->image))
                                <img src="{{ Str::startsWith($item->image, ['http://', 'https://', 'images/']) ? asset($item->image) : asset('storage/' . $item->image) }}" 
