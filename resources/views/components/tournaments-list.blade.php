...
<div class="gap-y-6.25 flex flex-col">
    @if(count($tournaments) > 0)
        @foreach ($tournaments as $item)
            <x-cards.tournament-list-item :tournament="$item" />
        @endforeach
    @else
        <div>
            <div
...
{{-- PAST TOURNAMENT CARDS LIST --}}
<div class="gap-y-6.25 flex flex-col">
    @if(count($pastTournaments) > 0)
        @foreach ($pastTournaments as $item)
            <x-cards.tournament-list-item :tournament="$item" />
        @endforeach
    @else
        <div
            class="rounded-[5px] border border-[#F3F3F3] px-4 py-6 text-center dark:border-none dark:bg-[#1F1F1F]">
            <p class="text-xs font-medium text-[#666]">No past tournaments available</p>
        </div>
    @endif
</div>
